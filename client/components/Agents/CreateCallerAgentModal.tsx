...
export function CreateCallerAgentModal({
    ...
    const [twilioConnections, setTwilioConnections] = useState<
        TwilioConnection[]
    >([]);
        ...
        <Modal open={open} title="Create caller agent" onClose={onClose}>
            <form onSubmit={handleSubmit(submit)} className="space-y-4">
                ...
                <Select
                    ...
                    {...register("twilio_connection_id", {
                        required: "Select a telephone provider",
                    })}
                >
                    <option value="">
                        {providersLoading
                            ? "Loading providers..."
                            : "Select telephone provider..."}
                    </option>
                ...
                <Select
                    ...
                    {...register("phone_number_id", {
                        required: "Select a phone number",
                    })}
                >
                    <option value="">
                        {phonesLoading
                ...
                <Select
                    ...
                    disabled={providersLoading}
                    {...register("elevenlabs_connection_id", {
                        required: "Select an AI provider",
                    })}
                >
                    <option value="">
                        {providersLoading
                            ? "Loading providers..."
                            : "Select ElevenLabs account..."}
                    </option>
                ...
                <Select
                    ...
                    {...register("elevenlabs_agent_id", {
                        required: "Select an ElevenLabs agent",
                    })}
                >
                    <option value="">
                        {agentsLoading
                ...
                <p className="text-xs text-zinc-500">
                    Registers the phone with ElevenLabs if needed, assigns the
                    selected agent, and saves a caller agent you can use in
                    campaigns.
                </p>

                <FormActions
                    onCancel={onClose}
                    submitLabel="Create caller agent"
                    loading={loading}
                />
            </form>
        </Modal>
