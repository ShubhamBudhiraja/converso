...
export function AddConnectionModal({ open, loading, onClose, onSubmit }: AddConnectionModalProps) {
  ...
  async function submit(data: AddConnectionForm) {
    ...
    try {
      ...
      setFormError(err instanceof ApiError ? err.message : "Failed to add ElevenLabs account");
    }
  }
    ...
    <Modal open={open} title="Add ElevenLabs account" onClose={onClose}>
      <form onSubmit={handleSubmit(submit)} className="space-y-4">
        ...
        <p className="text-xs text-zinc-500">
          Your API key is validated against ElevenLabs before being saved. To change the key later,
