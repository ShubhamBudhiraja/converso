...
export function ImportNumberModal({ open, loading, onClose, onSubmit }: ImportNumberModalProps) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  async function submit(data: ImportNumberForm) {
    ...
    try {
      ...
      setFormError(err instanceof ApiError ? err.message : "Failed to import phone number");
    }
  }
  ...
  return (
    <Modal open={open} title="Import phone number" onClose={onClose}>
      <form onSubmit={handleSubmit(submit)} className="space-y-4">
        ...
        <FormActions onCancel={onClose} submitLabel="Import number" loading={loading} />
      </form>
    </Modal>
  );
}
