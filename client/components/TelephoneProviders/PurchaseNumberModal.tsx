...
export function PurchaseNumberModal({
    ...
    const labelForm = useForm<{ label: string }>({
        defaultValues: { label: "" },
    });

    const [formError, setFormError] = useState<string | null>(null);
    const [selectedAvailable, setSelectedAvailable] =
        useState<TwilioAvailableNumber | null>(null);
    ...
    async function handleSearch(data: SearchForm) {
        ...
        try {
            ...
            if (numbers.length === 0) {
                setFormError(
                    "No available numbers found. Try different search filters.",
                );
            }
        } catch (err) {
            setFormError(
        ...
        <>
                ...
                <div className="space-y-4">
                    ...
                    >
                        ...
                        <Input
                            ...
                            label="Area code"
                            placeholder="415"
                            {...searchForm.register("area_code")}
                        />
                        <Select
                            id="number_type"
                            label="Type"
                            {...searchForm.register("number_type")}
                        >
                    ...
                    {availableNumbers.length > 0 ? (
                        <div className="max-h-48 overflow-y-auto rounded-lg border border-zinc-200 dark:border-zinc-800">
                            {availableNumbers.map((number) => {
                                const selected =
                                    selectedAvailable?.phone_number ===
                                    number.phone_number;
                                return (
                                    <button
                    ...
                    <p className="text-xs text-zinc-500">
                        Purchasing a number charges your Twilio account. You
                        will be billed by Twilio directly.
                    </p>
            ...
            <ConfirmDialog
                ...
                description={
                    <>
                        Purchase{" "}
                        <strong>{selectedAvailable?.phone_number}</strong> on
                        your Twilio account? This will incur charges on your
                        Twilio billing.
                    </>
                }
